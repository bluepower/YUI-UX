...
//------------------------------------------------------------
   ...
   // Sample namespace of GenMC Demeter project
   var Gcc = {
	   admin: {}
   };
...
//------------------------------------------------------------
	...
	Event.addListener(window, "load", function() {
		...
		var localJSON = function() {
			...
			// Create all the cell editor instances once at the beginning
			var editors = {
			...
			var fnDomainParser = function(data) {
				if (data) {
					return lang.JSON.parse(data);
				}
			};

			var fnRuleParser = function(data) {
				if (data) {
			...
			myDataSource.responseSchema = {
				...
				fields : [
					...
					"ApplyFrom",
					"ApplyTo",
					{key: "Domain", parser:fnDomainParser},
					{key: "Rule", parser:fnRuleParser},
					"RuleMsg",
					{key: "Readable", parser : "string"},
					{key: "Writable", parser : "string"}
				]
			};
			...
			myDataTable.subscribe("cellClickEvent", function (oArgs) {
				...
				if (column.editor == null || record.getData('Writable') == 'false') {
					return;
				}
				...
				// If dropdown, need to set dynamic options
				if (type == 8) {
					// Thanks for Jenny to support updating the dropdown options at runtime
					column.editor.dropdownOptions = record.getData('Domain');
			...
			// Do event subscriptions for cell editor instances at the beginning
